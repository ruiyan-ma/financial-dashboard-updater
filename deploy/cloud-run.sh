...
required_variables=(
  ...
  ASSETS_DATABASE_ID CURRENCIES_DATABASE_ID PLATFORMS_DATABASE_ID
  NET_VALUE_DATABASE_ID GROWTH_LOG_DATABASE_ID AI_SNAPSHOT_PAGE_ID
...
gcloud run jobs deploy "$UPDATER_JOB" \
  ...
  --max-retries 1 \
  --task-timeout 20m \
  --set-env-vars "ASSETS_DATABASE_ID=${ASSETS_DATABASE_ID},CURRENCIES_DATABASE_ID=${CURRENCIES_DATABASE_ID},PLATFORMS_DATABASE_ID=${PLATFORMS_DATABASE_ID},NET_VALUE_DATABASE_ID=${NET_VALUE_DATABASE_ID},GROWTH_LOG_DATABASE_ID=${GROWTH_LOG_DATABASE_ID},AI_SNAPSHOT_PAGE_ID=${AI_SNAPSHOT_PAGE_ID}" \
